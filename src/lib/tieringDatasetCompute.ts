...
type TieringCityBounds = {
    minCity: number;
    maxCity: number;
};

type TieringTimeBounds = {
    turnMin: number;
    turnMax: number;
    dayMin: number;
    dayMax: number;
...
function resolveCityBounds(data: GraphData): TieringCityBounds {
    let minCity = Number.MAX_SAFE_INTEGER;
    let maxCity = 0;
    for (let i = 0; i < data.coalitions.length; i++) {
        const cities = data.coalitions[i].cities;
        for (let j = 0; j < cities.length; j++) {
            const city = cities[j];
            if (city < minCity) minCity = city;
            if (city > maxCity) maxCity = city;
        }
    }
    return { minCity, maxCity };
}

function resolveTimeBounds(data: GraphData): TieringTimeBounds {
    let turnMin = Number.MAX_SAFE_INTEGER;
    let turnMax = 0;
    let dayMin = Number.MAX_SAFE_INTEGER;
    let dayMax = 0;
    for (let i = 0; i < data.coalitions.length; i++) {
        const coalition = data.coalitions[i];
        const turnRange = coalition.turn.range;
        const dayRange = coalition.day.range;
        if (turnRange[0] < turnMin) turnMin = turnRange[0];
        if (turnRange[1] > turnMax) turnMax = turnRange[1];
        if (dayRange[0] < dayMin) dayMin = dayRange[0];
        if (dayRange[1] > dayMax) dayMax = dayRange[1];
    }
    return { turnMin, turnMax, dayMin, dayMax };
}
...
export function getDataSetsByTime(
    ...
    return measureDetailedSyncTask(
        "main.graph.tiering.dataset.total",
        () => {
            const cityBounds = measureDetailedSyncTask(
                "main.graph.tiering.dataset.bounds",
                () => resolveCityBounds(data),
                {
            ...
            let minCity = cityBounds.minCity;
            let maxCity = cityBounds.maxCity;
            let normalizeAny = metrics.reduce((a, b) => a || b.normalize, false);
            let stackByAlliance = !normalizeAny && metrics.length == 1;
            const metricAccessors = measureDetailedSyncTask(
                "main.graph.tiering.dataset.metricAccessors",
                () => resolveMetricAccessors(data, metrics),
                {
            ...
            const timeBounds = resolveTimeBounds(data);
            ...
            let time_min = isAnyTurn ? timeBounds.turnMin : timeBounds.dayMin;
            let time_max = isAnyTurn ? timeBounds.turnMax : timeBounds.dayMax;
            let dataSetTimeLen = time_max - time_min + 1;

            measureDetailedSyncTask(
                "main.graph.tiering.dataset.collectRows",
                () => {
                    let jUsed = 0;
                    for (let i = 0; i < data.coalitions.length; i++) {
            ...
            const response = measureDetailedSyncTask(
                "main.graph.tiering.dataset.normalize",
                () => {
                    let nextResponse: TieringDataSet[] = new Array(len);
                ...
                },
                {
                    dataSetCount: len,
                    shouldGroupByBand,
                    bandCount: shouldGroupByBand ? bandCount : null,
                    cityCount,
                },
            );
        ...
        () => ({
            coalitionCount: data.coalitions.length,
            metricCount: metrics.length,
            requestedAllianceCount: alliance_ids.reduce((sum, ids) => sum + ids.length, 0),
            useSingleColor,
            cityBandSize,
        }),
    );
